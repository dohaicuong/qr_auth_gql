...
export const subscribeFn: SubscribeFunction = (request, variables, cacheConfig) => {
  return Observable.create(sink => {
    ...
    if (request.text) url.searchParams.append('query', request.text)
    if (request.id) url.searchParams.append('query', request.id)

    if (request.name) url.searchParams.append('operationName', request.name)
